...
class Attention(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        if self.rw:
            #value_layer: (bs, num_heads, num_patches, channels)
            #attention_probs: (bs, num_heads, num_patches, num_patches)
            alpha = self.config.rw.alpha
            num_patches = attention_probs.shape[-1]
            I = torch.eye(num_patches, device=attention_probs.device).unsqueeze(0).unsqueeze(0)
            context_layer = torch.linalg.solve(I - alpha * attention_probs, context_layer)
    ...
    def load_from(self, weights, n_block):
        ...
        with torch.no_grad():
            query_weight = np2th(weights[pjoin(ROOT, ATTENTION_Q,
                                                "kernel")]).view(self.hidden_size, self.hidden_size).t()
